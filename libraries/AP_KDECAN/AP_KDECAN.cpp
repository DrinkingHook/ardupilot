...
void AP_KDECAN::init()
{
    ...
    for (uint8_t i = 0; i < HAL_NUM_CAN_IFACES; i++)
    {
        ...
        if (CANSensor::get_driver_type(i) == AP_CAN::Protocol::KDECAN)
        {
            ...
            // 创建AP_KDECAN_Driver实例，并赋值给_driver
            return;
        }
    }
}
...
// #if HAL_WITH_ESC_TELEM // 如果定义了HAL_WITH_ESC_TELEM
//         // broadcast as request-telemetry msg to everyone    // 广播为请求遥测消息
//         if (_init.detected_bitmask != 0 && now_ms - _telemetry.timer_ms >= TELEMETRY_INTERVAL_MS)
//         {                                 // 如果检测到通道，并且距离上次发送遥测信息的时间间隔大于TELEMETRY_INTERVAL_MS
//                                           // if (send_packet(TELEMETRY_OBJ_ADDR, BROADCAST_NODE_ID, 10)) {          // 发送请求遥测信息
//             _telemetry.timer_ms = now_ms; // 更新上次发送遥测信息的时间
//                                           // }
//         }
// #endif // HAL_WITH_ESC_TELEM                        // 如果定义了HAL_WITH_ESC_TELEM

//         if ((_init.detected_bitmask == 0 || broadcast_esc_info_boot_spam_count > 0) && (now_ms - _init.detected_bitmask_ms >= broadcast_esc_info_next_interval_ms))
//         {
//             // broadcast an "anyone there?" quick at boot but then 1Hz forever until we see at least 1 esc respond     // 广播“有人吗？”快速启动，然后1Hz直到至少看到1个ESC响应
//             if (broadcast_esc_info_boot_spam_count > 0)
//             {                                         // 如果启动时发送3次ESC信息
//                 broadcast_esc_info_boot_spam_count--; // 减少启动时发送3次ESC信息计数
//             } else {
//                 broadcast_esc_info_next_interval_ms = 300; // 更新下次发送ESC信息间隔为50ms--------------------------------------------
//             }
//             // if (send_packet(ESC_INFO_OBJ_ADDR, BROADCAST_NODE_ID, 100000)) {
//             //     _init.detected_bitmask_ms = now_ms;
//             // }
...
bool AP_KDECAN_Driver::send_packet_uint16(const uint8_t address, const uint8_t dest_id, const uint32_t timeout_ms, const uint16_t data)
{
    ...
    // 调用send_packet函数发送数据包
    return send_packet(address, dest_id, timeout_ms, (uint8_t *)&data_be16, 2);
}

bool AP_KDECAN_Driver::send_packet(const uint8_t, const uint8_t dest_id, const uint32_t timeout_ms, const uint8_t *data, const uint8_t data_len)
{
    // 构造广播遥测请求帧的帧ID
    // broadcast telemetry request frame
    const frame_id_t id{
